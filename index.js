...
async function run() {
  try {
    ...
    app.post("/jwt", logger, async (req, res) => {
      ...
      res
        .cookie("token", token, {
          ...
          // sameSite: "none", //client site and server site is different
        })
        // .send(token);
        .send({ success: true });
    });
    ...
    app.get("/bookings", logger, verifyToken, async (req, res) => {
      ...
      // console.log("tok tok my token", req.cookies.token);
      let query = {};
      if (req.query?.email) {
        query = { email: req.query.email };
      }
      const result = await bookingCheckoutCollection.find(query).toArray();
      res.send(result);
    });
